...
if ! command -v uv >/dev/null 2>&1; then
  printf '%s\n' 'claudish-to-english configuration requires uv: https://docs.astral.sh/uv/getting-started/installation/' >&2
  exit 1
fi
...
exec uv run --no-project --no-config --managed-python --python 3.14 \
  "$plugin_root/scripts/configure.py" --plugin-data "$plugin_data"
