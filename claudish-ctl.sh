...
MODEL=""; PROVIDER=""
dbg() { :; }
# ponytail: no providers.sh in this fork -- rewrites always go through the
# claude CLI, so the only knob is which model alias it is handed.
PROVIDER="claude-cli"
...
provider_label() { printf 'claude CLI (this fork)'; }
...
case "$cmd" in
  ...
  last)
    ...
    # This fork rewrites through a nested `claude -p`, which writes its own
    # transcript under ~/.claude/projects/*claudish*cli-scratch/. Those are
    # rewrites, not the session, so the fallback must skip them or `last` can
    # print a rewrite back at the user as if it were the original.
    [ -n "$tp" ] || tp="$(ls -t "$HOME/.claude/projects"/*/*.jsonl 2>/dev/null \
                          | grep -v 'claudish.*cli-scratch' | head -n1)"
